    ...
    <main className="min-h-screen bg-white text-black dark:bg-black dark:text-white">
      ...
      <section className="relative h-[70vh] flex items-center overflow-hidden border-b border-gray-100 dark:border-gray-900">
        ...
        <div className="relative z-10 max-w-4xl px-8">
          ...
          {/* REMOVED WAVE: Changed to Internet connection */}
            ...
            {hostingPlans.map((plan) => (
              <Link 
                ...
                className={`group p-8 rounded-3xl border transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl flex flex-col justify-between ${
                  plan.popular ? 'border-blue-600 bg-blue-50/30 dark:bg-blue-900/10 shadow-lg' : 'border-gray-100 dark:border-gray-800 bg-white dark:bg-[#0A0A0A] hover:border-blue-400'
                }`}
              >
                <div>
                  ...
                  <div className="flex justify-between items-start mb-1">
                    ...
                    <Circle className="w-5 h-5 text-gray-300 group-hover:text-blue-600 transition-colors" />
                  </div>
                ...
                <div className={`w-full text-center py-3 rounded-xl font-bold text-sm transition-all ${
                  plan.popular ? 'bg-blue-600 text-white' : 'bg-black text-white dark:bg-white dark:text-black group-hover:bg-blue-600 group-hover:text-white'
                }`}>
                  Select {plan.name}
                </div>
              </Link>
      ...
      <section className="py-32 bg-gray-50 dark:bg-[#050505] border-y border-gray-100 dark:border-gray-900">
        <div className="container mx-auto px-8">
          <div className="grid lg:grid-cols-2 gap-20 items-center">
            ...
            <div>
              {/* REMOVED WAVE: Changed to Internet connectivity */}
              <h2 className="text-blue-600 font-bold tracking-widest text-sm mb-6 uppercase">Internet connectivity</h2>
        ...
        <div className="container mx-auto px-8">
          ...
          <div className="mt-20 p-12 bg-gray-50 dark:bg-[#0A0A0A] rounded-3xl border border-gray-100 dark:border-gray-900 flex flex-col md:flex-row items-center justify-between gap-8">
            <div className="max-w-xl">
              {/* REMOVED WAVE: Changed to Hotspot access */}
            ...
            <div className="flex gap-4">
              <Link href={{ pathname: '/support', query: { plan: 'Hotspot 1 Hour' } }} className="p-5 bg-white dark:bg-black border border-gray-200 dark:border-gray-800 text-center rounded-2xl w-28 shadow-sm hover:border-blue-600 transition-all">
                <p className="text-[10px] text-blue-600 font-bold uppercase mb-1">1 Hour</p>
                <p className="font-bold text-lg">KSh 10</p>
              </Link>
              <Link href={{ pathname: '/support', query: { plan: 'Hotspot 24 Hours' } }} className="p-5 bg-white dark:bg-black border border-gray-200 dark:border-gray-800 text-center rounded-2xl w-28 shadow-sm hover:border-blue-600 transition-all">
  ...
  return (
    <Link 
      ...
      className={`group p-6 border rounded-2xl flex items-center justify-between transition-all duration-300 hover:shadow-lg hover:translate-x-2 ${
        isPopular ? 'border-blue-600 bg-blue-50/50 dark:bg-blue-900/10' : 'border-gray-100 dark:border-gray-800 bg-white dark:bg-black hover:border-blue-400'
      }`}
    >
      <div className="flex items-center gap-4">
        <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors ${isPopular ? 'bg-blue-600 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-400 group-hover:bg-blue-600 group-hover:text-white'}`}>
...
function BusinessInternet({ pkg, speed, price, desc }: any) {
  return (
    <Link 
      ...
      className="group p-8 border border-gray-100 dark:border-gray-800 rounded-3xl bg-white dark:bg-[#0A0A0A] hover:border-blue-600 hover:shadow-xl transition-all flex flex-col justify-between"
    >
      <div>
        <h4 className="font-bold text-lg mb-2 group-hover:text-blue-600 transition-colors">{pkg}</h4>
      ...
      <div>
        ...
        <div className="flex items-center text-xs font-bold text-blue-600 mt-4">Select <ArrowRight className="ml-2 w-3 h-3 group-hover:translate-x-1 transition-transform" /></div>
      </div>
    </Link>
  );
}
...
function SSLCard({ type, price, desc }: any) {
  return (
    <Link 
      ...
      className="group p-10 border border-gray-100 dark:border-gray-800 rounded-3xl bg-white dark:bg-[#0A0A0A] hover:border-blue-600 hover:shadow-xl transition-all text-center flex flex-col items-center"
    >
      ...
      <div className="mt-auto">
        <div className="text-xl font-bold text-blue-600 mb-1">KSh {price}</div>
